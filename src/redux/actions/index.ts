...
export const setClusterData = createAction(
  ...
  (resolve) => (clusterData: FeatureCollection<Point, PointProperties>) => resolve({ clusterData }),
);

export const toggleDisplayTransmissionClusters = createAction(
  CONTROL_TOGGLE_DISPLAY_TRANSMISSION_CLUSTERS,
  (resolve) => (displayTransmissionClusters: boolean) => resolve({ displayTransmissionClusters }),
);

export const toggleDisplayCaseClusters = createAction(
  CONTROL_TOGGLE_DISPLAY_CASE_CLUSTERS,
  (resolve) => (displayCaseClusters: boolean) => resolve({ displayCaseClusters }),
);

export const setSelectedCluster = createAction(
  CONTROL_SET_SELECTED_CLUSTER,
  (resolve) => (selectedCluster: SelectedCluster | null) => resolve({ selectedCluster }),
);

export const setSelectedCase = createAction(
  CONTROL_SET_SELECTED_CASE,
  (resolve) => (selectedCase: SelectedCase | null) => resolve({ selectedCase }),
);

export const setDateRange = createAction(
  CONTROL_SET_END_DATE_RANGE,
  (resolve) => (numberOfDays: number) => resolve({ numberOfDays }),
);
...
export const updateServiceWorker = createAction(
  ...
  (resolve) => (serviceWorkerRegistration: ServiceWorkerRegistration) => resolve({ serviceWorkerRegistration }),
);

export const setModal = createAction(
  UI_SET_MODAL,
  (resolve) => (modal: Modal) => resolve({ modal }),
);
